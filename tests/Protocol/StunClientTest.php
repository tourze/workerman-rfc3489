 ...
 * @internal
 */
#[CoversClass(StunClient::class)]
final class StunClientTest extends TestCase
{
    /**
     * 测试正常情况下成功获取公网地址
     */
    public function testDiscoverPublicAddressSuccess(): void
    {
        // 创建模拟的传输层
     ...
     * 测试发送请求失败的情况
     */
    public function testDiscoverPublicAddressSendFailed(): void
    {
        // 创建模拟的传输层
        $transport = $this->createMock(StunTransport::class);
     ...
     * 测试接收响应超时的情况
     */
    public function testDiscoverPublicAddressReceiveTimeout(): void
    {
        // 创建模拟的传输层
        $transport = $this->createMock(StunTransport::class);
     ...
     * 测试接收响应但事务ID不匹配的情况
     */
    public function testDiscoverPublicAddressTransactionIdMismatch(): void
    {
        // 创建模拟的传输层
        $transport = $this->createMock(StunTransport::class);
        ...
        $this->expectExceptionMessageMatches('/无效的事务ID: [a-f0-9]+/');
     ...
     * 测试响应中没有映射地址信息的情况
     */
    public function testDiscoverPublicAddressNoMappedAddress(): void
    {
        // 创建模拟的传输层
        $transport = $this->createMock(StunTransport::class);
        ...
        // 预期异常
        $this->expectException(ProtocolException::class);
        $this->expectExceptionMessage('消息处理失败: 发现公网地址 - 响应中没有映射地址信息');
     ...
     * 测试服务器地址解析 - IP地址
     */
    public function testResolveServerAddressIpAddress(): void
    {
        // 创建模拟的传输层
        $transport = $this->createMock(StunTransport::class);
