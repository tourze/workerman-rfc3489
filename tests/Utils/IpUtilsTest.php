...
final class IpUtilsTest extends TestCase
{
    ...
    public function testEncodeAddressIpv6(): void
    {
        $ip = '2001:db8::1';
        $port = 8080;
        $encoded = IpUtils::encodeAddress($ip, $port);
    ...
    public function testDecodeAddressIpv4(): void
    {
        $binary = "\x00\x01\x1F\x90\xC0\xA8\x01\x01";
        [$ip, $port, $family] = IpUtils::decodeAddress($binary, 0);
    ...
    public function testDecodeAddressIpv6(): void
    {
        // 创建一个 IPv6 地址二进制数据样例
        $ipv6Binary = "\x00\x02\x1F\x90" . inet_pton('2001:db8::1');
        [$ip, $port, $family] = IpUtils::decodeAddress($ipv6Binary, 0);
    ...
    public function testDecodeAddressWithOffset(): void
    {
        $binary = "\x00\x00\x00\x01\x1F\x90\xC0\xA8\x01\x01";
        [$ip, $port, $family] = IpUtils::decodeAddress($binary, 2);

        $this->assertSame('192.168.1.1', $ip);
        $this->assertSame(8080, $port);
    }

    public function testDecodeAddressWithInvalidFamily(): void
    {
    ...
    public function testIsIpv4WithValidAddress(): void
    {
    ...
    public function testIsIpv4WithInvalidAddress(): void
    {
    ...
    public function testIsIpv6WithValidAddress(): void
    {
    ...
    public function testIsIpv6WithInvalidAddress(): void
    {
    ...
    public function testGetLocalIp(): void
    {
        ...
        // getLocalIp 可能返回 null，这是正常的
    }

    public function testFormatAddressPort(): void
    {
    ...
    public function testParseAddressPort(): void
    {
        [$ip, $port] = IpUtils::parseAddressPort('192.168.1.1:8080');
        $this->assertSame('192.168.1.1', $ip);
        $this->assertSame(8080, $port);

        [$ip, $port] = IpUtils::parseAddressPort('[2001:db8::1]:8080');
        $this->assertSame('2001:db8::1', $ip);
        $this->assertSame(8080, $port);

        [$ip, $port] = IpUtils::parseAddressPort('invalid');
        $this->assertNull($ip);
        $this->assertNull($port);
    }

    public function testIpEquals(): void
    {
